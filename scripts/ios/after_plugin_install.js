...
function updatePackageSwiftVersion(projectRoot, newVersion) {
    ...
    collectPackagePaths(path.join(projectRoot, "plugins"), 3);
    collectPackagePaths(path.join(projectRoot, "platforms", "ios", "packages"), 3);
    ...
    packagePaths.forEach(function(packagePath) {
        ...
        console.log("[FirebasexInAppMessaging] Firebase SDK version set to v" + newVersion + " in " + packagePath);
    });
}
...
function resolvePluginVariables(context) {
    ...
    if (!pluginVariables["IOS_FIREBASE_SDK_VERSION"]) {
        pluginVariables["IOS_FIREBASE_SDK_VERSION"] = DEFAULT_IOS_FIREBASE_SDK_VERSION;
    }

    if (!pluginVariables["IOS_FIREBASE_SDK_BETA_VERSION"]) {
        pluginVariables["IOS_FIREBASE_SDK_BETA_VERSION"] = DEFAULT_IOS_FIREBASE_SDK_BETA_VERSION;
    }
